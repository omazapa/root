...
// Rosenbrock(x,y=sin(y')+1)
double sfunc(const double* x)
{
	const double x_i[2] = { x[0], sin(x[1]) + 1. };
	return func(x_i);
}

double sgradf(const double* x, unsigned int i)
{
	const double x_i[2] = { x[0], sin(x[1]) + 1. };
	double ret = gradf(x_i, i);
	if(i == 1) ret *= cos(x[1]);
	return ret;
}
...
int main()
{
	ROOT::Math::GradFunctor gradFunctor(&sfunc, &sgradf, 2);
	...
	for(unsigned int i = 0; i < 2; ++i)
	{
		...
		std::cout << "  After " << mn->NCalls() << " function evaluations" << std::endl;

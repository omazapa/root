...
Long64_t TMVA::DataSet::GetNClassEvents( Int_t type, UInt_t classNumber ) 
{
   try {
      ...
      Log() << kFATAL << Form("[%s] : ",fdsi.GetName()) << "No " << (type==0?"training":(type==1?"testing":"_unknown_type_")) 
      ...
      Log() << kFATAL << Form("[%s] : ",fdsi.GetName()) << "ERROR/CAUGHT : DataSet/GetNClassEvents, .. unknown error" << Endl;
   }
   return 0;
}
                                   ...
                                   Types::EAnalysisType /* analysistype */ ) 
{
   ...
   if (UInt_t(type) > fResults.size()){
      Log()<<kFATAL<< Form("[%s] : ",fdsi.GetName()) << "you asked for an Treetype (training/testing/...)"
   ...
   if (it!=resultsForType.end()) {
      Log() << kDEBUG << Form("[%s] : ",fdsi.GetName()) << " Delete Results previous existing result:" << resultsName 
            << " of type " << type << Endl;
      delete it->second;
      resultsForType.erase(it->first);
   }
   else {
      Log() << kINFO << Form("[%s] : ",fdsi.GetName()) << "could not fine Result class of " << resultsName 
...
void TMVA::DataSet::CreateSampling() const
{
   ...
   if (fSamplingRandom == 0 )
      Log() << kFATAL<< Form("[%s] : ",fdsi.GetName()) 
...
void TMVA::DataSet::EventResult( Bool_t successful, Long64_t evtNumber )
{
   ...
   for ( Long64_t iEvt = start; iEvt <= stop; iEvt++ ){
      if (Long64_t(fSamplingEventList.at(fCurrentTreeIdx).size()) < iEvt) {
         Log() << kWARNING << Form("[%s] : ",fdsi.GetName()) << "event number (" << iEvt 
...
{ 
   ...
   Log() << kDEBUG << Form("[%s] : ",fdsi.GetName()) << "GetTree(" << ( type==Types::kTraining ? "training" : "testing" ) << ")" << Endl;
   ...
   if (fResults.size() <= t) {
      Log() << kWARNING << Form("[%s] : ",fdsi.GetName()) << "No results for treetype " << ( type==Types::kTraining ? "training" : "testing" ) 
   ...
   for (std::map< TString, Results* >::iterator itMethod = fResults.at(t).begin(); 
      ...
      Log() << kDEBUG << Form("[%s] : ",fdsi.GetName()) << "analysis type: " << (itMethod->second->GetAnalysisType()==Types::kRegression ? "Regression" :
      ...
      if (itMethod->second->GetAnalysisType() == Types::kClassification) {
         ...
         Log() << kDEBUG << Form("[%s] : ",fdsi.GetName()) << "itMethod->first " << itMethod->first <<  "    LEAFLIST: " 
         ...
         Log() << kWARNING << Form("[%s] : ",fdsi.GetName()) << "Unknown analysis type for result found when writing TestTree." << Endl;
      }
      n++;
   ...
   Log() << kINFO << Form("[%s] : ",fdsi.GetName()) << "Created tree '" << tree->GetName() << "' with " << tree->GetEntries() << " events" << Endl;

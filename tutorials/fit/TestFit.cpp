...
void TestFit()
{
   ...
   //std::vector<double>params = {1,1.,1,2.,1,2.,0.5};
   //fnorm_exp_gauss -> SetParameters(params.data());
   Int_t NofParams = fnorm_exp_gauss -> GetNpar();
   cout << " nofparams " << NofParams << endl;
   ...
   //{
      //fsum -> SetParameters(7e5, 1.5e5, 1.e5, Alpha, Mean1, Sigma1, Mean2, Sigma2);
   h_ExpGauss -> Fit("fsum");
   //for 1000 fit: Real time 0:10:47, CP time 646.200
   //}
   //for 10 : 6.37
   cout << "**************************************************" << endl;
   ...
   //DEBUG
   // fsum -> Print();
   //cout << "Number of parameters : " << fnorm_exp_gauss -> GetNpar() << endl;
   ...
   // from a tree:
   // RooDataSet data("data","mydata",w::x,myTree);
   //from an histogram:¨
   ...
   frame->Draw();
// gaussian::blabla is the SAME AS: RooRealVar x("x","x",-5.,5.);RooRealVar m("m","mean",1.);RooRealVar s("s","sigma",0.3);RooGaussian g("g","g(x,m,s),x,m,s)
}
